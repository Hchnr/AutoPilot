...
def generate_reconcile_report(result: ReconcileResult) -> str:
    ...
    if analysis.get("status") == "no_data":
        ...
        if "ttft" in analysis:
            ttft = analysis["ttft"]
            lines.append(f"- TTFT: avg={ttft['avg']:.0f}ms, max={ttft['max']:.0f}ms, trend={ttft['trend']}")
        if "itl" in analysis:
            itl = analysis["itl"]
            lines.append(f"- ITL: avg={itl['avg']:.0f}ms, max={itl['max']:.0f}ms, trend={itl['trend']}")
        if "gpu_utilization" in analysis:
            gpu = analysis["gpu_utilization"]
            lines.append(f"- GPU utilization: avg={gpu['avg']:.0%}, max={gpu['max']:.0%}")
        if "kv_cache_utilization" in analysis:
            kv = analysis["kv_cache_utilization"]
            lines.append(f"- KV cache utilization: avg={kv['avg']:.0%}, max={kv['max']:.0%}")
        if "errors" in analysis:
            err = analysis["errors"]
            lines.append(f"- OOM events: {err['total_oom']}, error rate: {err['avg_error_rate']:.4f}")
    lines.append("")
    ...
    if not result.actions:
        lines.append("**No action needed.** Current deployment is within acceptable parameters.\n")
    else:
        for i, action in enumerate(result.actions, 1):
            lines.append(f"### Action {i}: {action.action}\n")
            lines.append(f"- **Change**: `{action.field}` from `{action.from_value}` to `{action.to_value}`")
            ...
            if action.requires_restart:
                lines.append("- ⚠️ **Requires restart**: This change requires service restart and may cause brief downtime")
            lines.append("")

    # Safety Notes
    lines.append("## Safety Notes\n")
    lines.append("- Actions are limited to one per reconcile cycle to avoid compounding changes")
    lines.append("- Scale-down operations require more consecutive windows of low utilization")
