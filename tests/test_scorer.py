...
def workload():
    return WorkloadSummary(
        ...
        input_tokens_p90=3000,
        input_tokens_p99=5000,
        output_tokens_p50=300,
        output_tokens_p90=500,
        output_tokens_p99=800,
        avg_rps=10.0,
        peak_rps=15.0,
        burst_ratio=1.5,
...
class TestScorer:
    ...
    def test_slo_hard_constraint(self, profiles, cluster, workload):
        ...
        ranked = score_and_rank(
            candidates, workload, strict_slo, profile_lookup, cluster
        )
        # 极严格 SLO → 可能无法满足
        # 这取决于估算，但至少验证不崩溃
    ...
    def test_at_least_scored(self, profiles, cluster, workload):
        ...
        slo = SloConfig(
            ...
            constraints=SloConstraints(
                p95_ttft_ms=2000, p95_itl_ms=100, minimum_capacity_headroom=0.0
            ),
        )
        profile_lookup = ProfileLookup(profiles=profiles)
    ...
    def test_slo_filters_violating_plans(self, profiles, cluster, workload):
        ...
        strict_slo = SloConfig(
            objective=SloObjective(primary="minimize_hourly_cost"),
            constraints=SloConstraints(
                p95_ttft_ms=50, p95_itl_ms=5, minimum_capacity_headroom=0.0
            ),
        )
        profile_lookup = ProfileLookup(profiles=profiles)
    ...
    def test_goodput_ranking_differs_from_cost(self, profiles, cluster):
        ...
        cost_slo = SloConfig(
            objective=SloObjective(primary="minimize_hourly_cost"),
            constraints=SloConstraints(
                p95_ttft_ms=3000, p95_itl_ms=200, minimum_capacity_headroom=0.0
            ),
        )
        ...
        workload = WorkloadSummary(
            input_tokens_p50=500,
            input_tokens_p90=1000,
            input_tokens_p99=1500,
            output_tokens_p50=100,
            output_tokens_p90=200,
            output_tokens_p99=300,
            avg_rps=2.0,
            peak_rps=3.0,
            burst_ratio=1.5,
        ...
        ranked_cost = score_and_rank(
            candidates, workload, cost_slo, profile_lookup, cluster
        )
        ranked_goodput = score_and_rank(
            candidates, workload, goodput_slo, profile_lookup, cluster
        )

        if len(ranked_cost) >= 2 and len(ranked_goodput) >= 2:
            # minimize_cost 应更偏好 1 副本（便宜）
            assert (
                ranked_cost[0].estimated_hourly_cost
                <= ranked_cost[1].estimated_hourly_cost
            )
