...
def cluster():
    return ClusterSpec(
        gpu_pools=[
            ...
            GpuPool(id="l40s", gpu_type="L40S-48GB", count=16, memory_gb=48, topology="pcie", hourly_cost_per_gpu=1.5),
        ]
    )
...
def workload():
    return WorkloadSummary(
        input_tokens_p50=2000, input_tokens_p90=4000, input_tokens_p99=6000,
        output_tokens_p50=300, output_tokens_p90=600, output_tokens_p99=1000,
        avg_rps=10.0, peak_rps=20.0, burst_ratio=2.0,
        prefix_reuse_rate=0.8, total_requests=500, requests_with_prefix=400,
        estimated_concurrency=8.0,
        is_prefill_heavy=True, is_latency_sensitive=True,
        has_time_pattern=True, peak_window_rps=20.0, valley_window_rps=5.0,
    )
...
class TestCandidateGenerator:
    ...
    def test_infeasible_memory_excluded(self, model, cluster, backends, workload, slo):
        ...
        for c in candidates:
            if c.gpu_pool == "l40s" and c.precision == "bf16" and c.tensor_parallel == 1:
    ...
    def test_prefix_cache_influenced_by_workload(self, model, cluster, backends, slo):
        ...
        candidates_high = generate_candidates(model, cluster, backends, high_prefix, estimator, slo)
        ...
        low_prefix = WorkloadSummary(
            input_tokens_p50=200, input_tokens_p90=500, input_tokens_p99=800,
            output_tokens_p50=1500, output_tokens_p90=2500, output_tokens_p99=3500,
            avg_rps=5.0, peak_rps=8.0, burst_ratio=1.6,
            prefix_reuse_rate=0.05, total_requests=300, requests_with_prefix=10,
            estimated_concurrency=5.0,
            is_prefill_heavy=False, is_latency_sensitive=False,
            has_time_pattern=False,
        )
